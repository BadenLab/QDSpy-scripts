...
p = {"nTrials"         : 6,
     ...
     "vel_umSec"       : 300.0, # speed of moving bar in um/sec
     "tMoveDur_s"      : 5.0,    # duration of movement (defines distance
                                 # the bar travels, not its speed)
     "barDx_um"        : bar_length,  # bar dimensions in um
     "barDy_um"        : bar_width,
...
def MoveBarSeq():
  ...
  for rot_deg in p["DirList"]:
    ...
    if iStep == max(range(round(nFrToMove))):
        QDS.Scene_RenderEx(p["Wait_dur"], [1], [(x,y)], [(0.0,0.0)], [rot_deg])
